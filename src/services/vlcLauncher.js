...
@="powershell -windowstyle hidden -command \\"$u='%1'; $filePath=''; $subPath=''; $fMatch=[regex]::Match($u, '[?&]f=([^&]+)'); if ($fMatch.Success) { $fRaw=[System.Uri]::UnescapeDataString($fMatch.Groups[1].Value); try { $filePath=[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($fRaw)) } catch {} }; if (-not $filePath) { $pMatch=[regex]::Match($u, '[?&]path=([^&]+)'); if ($pMatch.Success) { $filePath=[System.Uri]::UnescapeDataString($pMatch.Groups[1].Value) } }; $sMatch=[regex]::Match($u, '[?&]s=([^&]+)'); if ($sMatch.Success) { $sRaw=[System.Uri]::UnescapeDataString($sMatch.Groups[1].Value); try { $subPath=[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($sRaw)) } catch {} }; if (-not $subPath) { $subMatch=[regex]::Match($u, '[?&]sub=([^&]+)'); if ($subMatch.Success) { $subPath=[System.Uri]::UnescapeDataString($subMatch.Groups[1].Value) } }; if ($filePath) { $filePath=$filePath -replace '^file:///?', '' -replace '%5[cC]', '\\\\' -replace '%20', ' ' -replace '/', '\\\\' }; if ($subPath) { $subPath=$subPath -replace '^file:///?', '' -replace '%5[cC]', '\\\\' -replace '%20', ' ' -replace '/', '\\\\' }; $targetFile=$filePath; if ($targetFile -and -not (Test-Path -LiteralPath $targetFile)) { $cands=@(\\"$env:USERPROFILE\\\\Downloads\\\\$targetFile\\", \\"$env:USERPROFILE\\\\Movies\\\\$targetFile\\", \\"$env:USERPROFILE\\\\Videos\\\\$targetFile\\", \\"$env:USERPROFILE\\\\Desktop\\\\$targetFile\\", \\"C:\\\\$targetFile\\", \\"D:\\\\$targetFile\\", \\"E:\\\\$targetFile\\"); foreach ($c in $cands) { if (Test-Path -LiteralPath $c) { $targetFile=$c; break } } }; if ($targetFile -and (Test-Path -LiteralPath $targetFile)) { $vlcPaths=@('C:\\\\Program Files\\\\VideoLAN\\\\VLC\\\\vlc.exe', 'C:\\\\Program Files (x86)\\\\VideoLAN\\\\VLC\\\\vlc.exe', \\"$env:LOCALAPPDATA\\\\Programs\\\\VLC\\\\vlc.exe\\"); $vlcExe=$null; foreach ($v in $vlcPaths) { if (Test-Path -LiteralPath $v) { $vlcExe=$v; break } }; if ($vlcExe) { $vlcArgs='\\\\\\"' + $targetFile + '\\\\\\\"'; if ($subPath -and (Test-Path -LiteralPath $subPath)) { $vlcArgs += ' --sub-file=\\\\\\\"' + $subPath + '\\\\\\\"' }; Start-Process -FilePath $vlcExe -ArgumentList $vlcArgs } else { Start-Process -FilePath $targetFile } } else { Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Media file not found at local path:\`n' + $filePath + '\`n\`nPlease update local file path in Film Library.', 'Film Library - File Not Found') }\\""
`;
