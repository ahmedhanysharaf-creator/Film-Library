...
psFile.WriteLine "    $argList = @('--sub-autodetect-file', '--sub-autodetect-fuzzy=1', '--sub-track=0', '--sub-track-id=1', '--sub-language=ar,en,any', $arg1)"
psFile.WriteLine "    if ($url -match 'sub=(.*?)(?:&|$)') {"
psFile.WriteLine "        $s = [System.Uri]::UnescapeDataString($matches[1]).Replace('/', '\\')"
psFile.WriteLine "        $arg2 = '""--sub-file=' + $s + '""'"
